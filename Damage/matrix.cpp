...
int  MATRIX::BinaryToDecimal(int i, int j, int ln) {
	...
	for (int wd=j;wd<(j+ln);wd++) {
		...
		totalValue = totalValue + placeValue*Get(i,wd);
...
int  MATRIX::ComputeFSM(MATRIX *str, MATRIX *states, int i, int strIndex, int strLength, int q0, MATRIX *F) {
	...
	if ( strIndex == strLength ) {
		...
		int currChar = str->Get(i,strIndex);

		return( ComputeFSM(str,states,i,++strIndex,strLength,Get(currChar,q0),F) );
	}
}
...
double MATRIX::GetPressure(int whichFoot) {

	return( Get(length-1,NUM_ANGLE_SENSORS+whichFoot) );
}
...
double MATRIX::GetTouch(int whichFoot) {

	if ( Get(length-1,NUM_ANGLE_SENSORS+whichFoot) > 0.0 )
		return( 1.0 );
	else
		return( 0.0 );
}
...
void MATRIX::PostProcess(MATRIX *other) {

	double initialSensorValue;

	for (int j=NUM_ANGLE_SENSORS;j<NUM_ANGLE_SENSORS+NUM_TOUCH_SENSORS;j++) {

		initialSensorValue = other->Get(0,j);

		MultColumn(j,-1.0);

		printf("%3.3f\n",initialSensorValue-Get(0,j));

		AddToColumn(j,initialSensorValue - Get(0,j));
	}
}

void MATRIX::PostProcessAngles(void) {
	...
	for (j=0;j<8;j=j+2) {
		...
		for (i=0;i<length;i++) {
			...
			if ( Get(i,j) == 0.0 )
				...
				angle = simParams->Scale(Get(i,j),MIN_RANGE_OF_MOTION,MAX_RANGE_OF_MOTION,-3.14159/2.0,3.14159/2.0);
...
void MATRIX::Reduce(int amt) {
	...
	double *newVals = new double[newLength*width];
...
void MATRIX::RoundDownColumn(int j) {

	for (int i=0;i<length;i++)

		Set(i,j,int(floor(Get(i,j))));
}
...
int  MATRIX::RowDifference(int myRow, int hisRow, MATRIX *m) {
	...
	for (int j=0;j<width;j++)
		diff = diff + abs( Get(myRow,j) - m->Get(hisRow,j) );
...
int  MATRIX::BinaryToDecimal(int j) {
	...
	for (int i=0;i<length;i++) {
		...
		totalValue = totalValue + placeValue*Get(i,j);
		placeValue = placeValue*2;
	}
