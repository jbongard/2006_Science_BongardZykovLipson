...
extern int    DAMAGE_TYPES;
extern double MAX_MASS_CHANGE;
...
void GENOME::WriteBody(ofstream *outFile) {
	...
	(*outFile) << raw->length + NUM_TOUCH_SENSORS   << " ";
	(*outFile) << (raw->length-1) + NUM_TOUCH_SENSORS << "\n\n";
	
	double lowestPoint = 0.4;

	for (int currObj=0;currObj<raw->length;currObj++)
		WriteObject(currObj,outFile,lowestPoint);

	for (int currJoint=1;currJoint<raw->length;currJoint++)
...
void GENOME::CreateEstimationGenome(void) {
	...
	raw = new MATRIX(NUM_STICKS,2);

	raw->MultColumn(0,DAMAGE_TYPES+1);

	raw->RoundDownColumn(0);

	raw->MultColumn(1,2.0*MAX_MASS_CHANGE);

	raw->AddToColumn(1,-MAX_MASS_CHANGE);
...
void GENOME::WriteJoint(int j, ofstream *outFile, double lowestPoint) {
	...
	(*outFile) << "-jointPosition ";
		...
		switch ( j ) {
		case 1:
			(*outFile) << "-0.7 0.4 0.293135\n";
			break;
		case 2:
			(*outFile) << "0.177386 0.4 0.7\n";
			break;
		case 3:
			(*outFile) << "0.7 0.4 0.000305683\n";
			break;
		case 4:
			(*outFile) << "-0.118469 0.4 -0.7\n";
			break;
		case 5:
			(*outFile) << "-1.59286 0.4 0.581761\n";
			break;
		case 6:
			(*outFile) << "0.373204 0.4 1.63286\n";
			break;
		case 7:
			(*outFile) << "1.65 0.4 -0.00893424\n";
			break;
		case 8:
			(*outFile) << "-0.236055 0.4 -1.64051\n";
			break;
		}
	...
	(*outFile) << "-jointNormal ";
		...
		switch ( j ) {
		case 1:
			(*outFile) << "0.44656 0 1.14954\n";
			break;
		case 2:
			(*outFile) << "1.16562 0 -0.271301\n";
			break;
		case 3:
			(*outFile) << "0.000469226 0 -1.175\n";
			break;
		case 4:
			(*outFile) << "-1.17081 0 0.18142\n";
			break;
		case 5:
			(*outFile) << "0.46968 0 0.794531\n";
			break;
		case 6:
			(*outFile) << "0.917299 0 -0.282641\n";
			break;
		case 7:
			(*outFile) << "0.094083 0 -0.948827\n";
			break;
		case 8:
			(*outFile) << "-0.953956 0 0.0278043\n";
			break;
		}
...
void GENOME::WriteObject(int o, ofstream *outFile, double lowestPoint) {
	...
	(*outFile) << "-position ";
		...
		case 0:
			...
			break;
		case 1:
			(*outFile) << "-1.14954 0.4 0.44656\n";
			break;
		case 2:
			(*outFile) << "0.271301 0.4 1.16562\n";
			break;
		case 3:
			(*outFile) << "1.175 0.4 0.000469226\n";
			break;
		case 4:
			(*outFile) << "-0.18142 0.4 -1.17081\n";
			break;
		case 5:
			(*outFile) << "-1.94407 0.4 0.91624\n";
			break;
		case 6:
			(*outFile) << "0.553942 0.4 2.08292\n";
			break;
		case 7:
			(*outFile) << "2.12383 0.4 0.0945522\n";
			break;
		case 8:
			(*outFile) << "-0.209224 0.4 -2.12477\n";
			break;
		}
	...
	(*outFile) << "-rotation ";
		...
		case 0:
			...
			break;
		case 1:
			(*outFile) << "-0.899079 0 0.30685\n";
			break;
		case 2:
			(*outFile) << "0.18783 0 0.931246\n";
			break;
		case 3:
			(*outFile) << "0.95 0 0.000327087\n";
			break;
		case 4:
			(*outFile) << "-0.125902 0 -0.94162\n";
			break;
		case 5:
			(*outFile) << "-0.702422 0 0.668957\n";
			break;
		case 6:
			(*outFile) << "0.361477 0 0.90013\n";
			break;
		case 7:
			(*outFile) << "0.947661 0 0.206973\n";
			break;
		case 8:
			(*outFile) << "0.0536616 0 -0.968515\n";
			break;
		}
	...
	(*outFile) << "-shape ";
		...
		if ( o==0 )
			(*outFile) << "-rectangle 0.82 0.82 0.9\n";
		
		else if ( o <= 4 )
			(*outFile) << "-cylinder 0.07 0.95\n";

		else
			(*outFile) << "-cylinder 0.07 0.97\n";
	...
	(*outFile) << "-mass ";
		...
		else if ( raw->Get(o,0)==1 ) // Damage: piece missing
			(*outFile) << "0.0001\n";
	...
	if ( raw->Get(o,0)!=1 ) // Damage: piece missing
	...
	if ( o>=5 ) {
		...
		(*outFile) << "-shape -rectangle 0.37 0.3 0.4\n";
