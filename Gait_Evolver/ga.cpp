...
void GA::ComputeTestFitnesses(void) {
	...
	for (int i=0;i<numOfResults;i++) {
		...
		//double totalDist = sqrt( pow(testResults[i]->Get(0,15),2.0) + 
		//						 pow(testResults[i]->Get(0,16),2.0) );

		double totalDist = testResults[i]->Get(0,15);

		genomes[i]->fitness = 0.0;

		for (int j=1;j<15;j++)

			genomes[i]->fitness = genomes[i]->fitness + testResults[i]->Get(0,j);

		/*
		genomes[i]->fitness =	testResults[i]->Get(0,18) +		// Penalty for tipping
								-testResults[i]->Get(0,17) +	// Reward for total height
								-totalDist;						// Reward for total distance

		genomes[i]->fitness =	genomes[i]->fitness +			// Penalty for slipping
								-testResults[i]->Get(0,17);		// Reward for maximum height
		*/
		genomes[i]->fitness =	testResults[i]->Get(0,18) +		// Penalty for tipping
								-totalDist;						// Reward for total distance
	}
...
void GA::OpenDataFiles(int append) {
	...
	genomeFile = new ofstream(genomeFileName,ios::app);
}
...
void GA::WriteBest(void) {
	...
	int currentJoint, i;
	double currCommand;
	double currMotorCommand;

	MATRIX *motorProgram = new MATRIX(MOTOR_PROGRAM_CYCLES+1,NUM_STICKS-1,0.0);

	for (currentJoint=0;currentJoint<NUM_STICKS-1;currentJoint++) {

		currMotorCommand = 0;
		/*
		if ( (currentJoint==0) || (currentJoint==4) )
			currMotorCommand = 0;
		if ( (currentJoint==1) || (currentJoint==5) )
			currMotorCommand = 1;
		if ( (currentJoint==2) || (currentJoint==6) )
			currMotorCommand = 2;
		if ( (currentJoint==3) || (currentJoint==7) )
			currMotorCommand = 3;
		*/

		for (i=1;i<MOTOR_PROGRAM_CYCLES;i++) {

			currCommand = genomes[0]->nn->motorCommands->Get(currMotorCommand++,currentJoint);
			//if ( currentJoint < 4 )
			//	currCommand = genomes[0]->nn->motorCommands->Get(currMotorCommand++,0);
			//else
			//	currCommand = genomes[0]->nn->motorCommands->Get(currMotorCommand++,1);

			if ( currMotorCommand == 8 )
				currMotorCommand = 0;

			currCommand = simParams->Scale(	currCommand,0.0,1.0,
											MIN_RANGE_OF_MOTION,MAX_RANGE_OF_MOTION);

			motorProgram->Set(i,currentJoint,currCommand);
		}
...
void GA::WriteEstReports(void) {
	...
	// Create stick phylogeny file
	sprintf(fileName,"%s_%d_%d_sticks.dat",EST_FILENAME,simParams->randSeed,simParams->GetRegimeIndex());
	simParams->stickPhylogenyFile = new ofstream(fileName,ios::app);

	genomes[0]->SaveSticks(simParams->stickPhylogenyFile);
...
void GA::WriteModelForEvaluation(int currentModel, ofstream *bodyFile) {
	...
	if ( simParams->performingIntelligentTesting )
		sprintf(fileName,"..\\..\\Data_t16_0_29\\est_0_2_body15.dat");
	else
		sprintf(fileName,"..\\..\\Data_t16_0_29\\est_0_1_body15.dat");
	*/
	sprintf(fileName,"..\\..\\..\\Data\\Bodies_To_Use\\Body.dat");

	ifstream *inBodyFile = new ifstream(fileName);
	...
	do {
		...
		if ( (temp[0]=='(') ||
			 ...
			 ((temp[0]=='-') && (temp[1]=='e')) ||
			 ((temp[0]=='-') && (temp[1]=='t')) ||
			 ((temp[0]=='-') && (temp[1]=='c') && (temp[2]!='y')) )
			(*bodyFile) << "\n";

		if ( !inBodyFile->eof() )
			(*bodyFile) << temp << " ";

	} while ( !inBodyFile->eof() );
...
void GA::WriteModelsForEvaluation(MATRIX **models) {
	...
	for (i=0;i<popSize;i++) {
		...
		WriteModelForEvaluation(i,bodyFile);
		simParams->totalModelEvals++;
	}
...
MATRIX *GA::WriteMotorProgramForEvaluation(int currentTest) {
	...
	int currentJoint, j, k, el;
	double currCommand;
	int	currMotorCommand;

	MATRIX *motorProgram = new MATRIX(EVAL_PERIOD,NUM_STICKS-1,0.0);
	...
	for (currentJoint=0;currentJoint<NUM_STICKS-1;currentJoint++) {
		...
		*/
		...
		k = 0;
		startCommand = 0.0;
		endCommand = genomes[currentTest]->nn->motorCommands->Get(currMotorCommand++,currentJoint);
		//endCommand = genomes[currentTest]->nn->motorCommands->Get(currMotorCommand++,0);
		/*
		if ( currentJoint < 4 )
			endCommand = genomes[currentTest]->nn->motorCommands->Get(currMotorCommand++,0);
		else
			endCommand = genomes[currentTest]->nn->motorCommands->Get(currMotorCommand++,1);
		*/

		if ( currMotorCommand == 4 )
			currMotorCommand = 0;

		endCommand = simParams->Scale(endCommand,0.0,1.0,
												 MIN_RANGE_OF_MOTION,MAX_RANGE_OF_MOTION);

		currCommand = startCommand;
		for (j=0;j<(EVAL_PERIOD/MOTOR_PROGRAM_CYCLES);j++) {

			motorProgram->Set(k,currentJoint,currCommand);

			currCommand = currCommand + (endCommand-startCommand)/
				                        ((EVAL_PERIOD/MOTOR_PROGRAM_CYCLES)-1);
			k++;
		}

		for (el=0;el<(MOTOR_PROGRAM_CYCLES-1);el++) {

			/*
			startCommand	= genomes[currentTest]->nn->motorCommands->Get(currMotorCommand++,currentJoint);
			startCommand	= simParams->Scale(startCommand,0.0,1.0,
														  MIN_RANGE_OF_MOTION,MAX_RANGE_OF_MOTION);
			*/
			startCommand = endCommand;

			if ( el >= ((MOTOR_PROGRAM_CYCLES-1)-1) ) {
...
void GA::WriteMotorProgramForOutput(int currentTest) {
	...
	char fileName[200];
	sprintf(fileName,"%s_%d_%d_brain%d_full.dat",EXP_FILENAME,simParams->randSeed,simParams->GetRegimeIndex(),simParams->currentCycle);

	ofstream *outFile = new ofstream(fileName);

	(*outFile) << "1\n";
	motorProgram->Write(outFile);
