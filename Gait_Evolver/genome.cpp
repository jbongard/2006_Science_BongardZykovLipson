...
void GENOME::WriteJoint(int j, ofstream *outFile, double lowestPoint) {
	...
	(*outFile) << "-jointPosition ";
		...
		(*outFile) << attachPos->Get(j,1) + lowestPoint + (1.0) << " ";
...
void GENOME::WriteObject(int o, ofstream *outFile, double lowestPoint) {
	...
	(*outFile) << "-position ";
		...
		if ( o==0 )
			(*outFile) << (stickPos->Get(o,2)+stickPos->Get(o,3))/2.0 + (0.9/2.0) + (1.0) << " ";
		else
			(*outFile) << (stickPos->Get(o,2)+stickPos->Get(o,3))/2.0 + lowestPoint  + (1.0) << " ";
