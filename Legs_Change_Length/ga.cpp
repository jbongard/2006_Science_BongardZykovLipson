...
void GA::InitGA(void) {
	...
	if ( simParams->performingEstimation ) {
		...
		if ( simParams->performingBatch )
			totalGenerations = int(simParams->numEstGenerations * 8.5);
		else
...
void GA::WriteEstReports(void) {
	...
	// Save leg lengths
		sprintf(fileName,"%s_%d_%d_genomes.dat",EST_FILENAME,simParams->randSeed,simParams->GetRegimeIndex());
		simParams->genomeFile = new ofstream(fileName,ios::app);

		genomes[0]->SaveGenome(simParams->genomeFile);

		simParams->genomeFile->close();
		delete simParams->genomeFile;
		simParams->genomeFile = NULL;
